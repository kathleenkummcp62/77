...
export function useWebSocket(url?: string) {
  ...
  const getWebSocketUrl = useCallback(() => {
    ...
    const port = envPort || window.location.port || "8080";
    ...
    try {
      ...
      wsRef.current.onclose = (event) => {
        ...
        if (
          ...
          setError(
            "Failed to connect to server. Please check if the Go server is running on port 8080.",
          );
        }
      };
    ...
    try {
      switch (message.type) {
        ...
        case "scanner_started":
          ...
          toast.success(
            `Scanner started: ${message.data.vpn_type || "Unknown"}`,
          );
          break;
        ...
        case "scanner_stopped":
          ...
          toast.success(
            `Scanner stopped: ${message.data.vpn_type || "Unknown"}`,
          );
          break;
